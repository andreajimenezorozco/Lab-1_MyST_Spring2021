...
def key_metrics(active, passive, rf):

    # Inversion Pasiva
    rend_avg_passive = round((passive['Rend. (%)'].mean()) * 100, 2)
    rendacum_avg_passive = round((passive['Rend. Acum (%)'].mean()) * 100, 2)
    sharpe_passive = round((passive['Rend. (%)'].mean() - rf) / passive['Rend. (%)'].std(), 2)

    # Inversion Activa
    rend_avg_active = round((active['Rend (%)'].mean()) * 100, 2)
    rendacum_avg_active = round((active['Rend (%) Acum.'].mean()) * 100, 2)
    sharpe_active = round((active['Rend (%)'].mean() - rf) / active['Rend (%)'].std(), 2)

    data = [['rend_m', 'Rendimiento promedio mensual', str(rend_avg_passive) + "%", str(rend_avg_active) + "%"],
            ['rend_c', 'Rendimiento mensual acumulado', str(rendacum_avg_passive) + "%", str(rendacum_avg_active) + "%"],
            ['sharpe', 'Sharpe ratio', sharpe_passive,sharpe_active]]

    metrics = pd.DataFrame(data, columns=['Medida', 'Descripción', 'Inversion Pasiva', 'Inversion Activa'])
